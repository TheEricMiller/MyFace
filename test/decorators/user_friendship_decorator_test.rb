...
class UserFriendshipDecoratorTest < Draper::TestCase
	...
	context "#sub_message" do
		...
		setup do
			@friend = create(:user, first_name: 'UserFname')
		end
	  ...
	  context "with a pending user friendship"  do
		...
		should "return the coorect message" do
		  assert_equal "Do you really want to be friends with UserFname?", @decorator.sub_message
		end
	  ...
	  context "with an accepted user friendship"  do
		...
		should "return the correct messge" do
			assert_equal "You are friends with UserFname.", @decorator.sub_message
		end
	  end
	end
